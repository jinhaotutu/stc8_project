...
#include    "STC8HX.h"       //包含此头文件后，里面声明的寄存器不需要再手动输入，避免重复定义
#include    "intrins.h"
...
void main(void)
{
    ...
    i = e2prom_write(0x0000,&RX1_Buffer[9],j);      //写N个字节
    if(i == 0)
    {
        PrintString1("已写入");
                ...
                if((RX1_Cnt >= 10) && (RX1_Buffer[1] == ' ') && (RX1_Buffer[8] == ' ')) //最短命令为10个字节
                {
                    ...
                    if(addr < 63488)    //限制在0~123扇区
                    {
                        if(RX1_Buffer[0] == 'W')    //写入N个字节
                        {
                            ...
                            i = EEPROM_write_n(0x0000,&RX1_Buffer[9],j);      //写N个字节
                            if(i == 0)
                            {
                                PrintString1("已写入");
                                if(j >= 100)    {UART1_TxByte(j/100+'0');   j = j % 100;}
